...
std::vector<std::string> get_day_folders() {
    ...
    for (const auto& entry : fs::directory_iterator(".")) {
        if (entry.is_directory() && entry.path().filename().string().substr(0, 3) == "day") {
...
double benchmark_program(const std::string& day_folder, const std::string& part) {
...
void write_readme(const std::vector<std::string>& day_folders, const std::vector<std::pair<double, double>>& results) {
    std::ofstream readme("README.md");
    readme << "# Advent of Code 2025\n\n";
    readme << "This repository contains my solutions for Advent of Code 2025, written in C++.\n\n";
    ...
    for (size_t i = 0; i < day_folders.size(); ++i) {
        readme << "| " << day_folders[i] << "  | "
               << (results[i].first >= 0 ? std::to_string(results[i].first) : "Error") << " | "
               << (results[i].second >= 0 ? std::to_string(results[i].second) : "Error") << " |\n";
    }

    readme << "\n---\n";
    readme << "### How to Run the Benchmark\n";
    readme << "1. Place the benchmark program in the root of this repository.\n";
    readme << "2. Compile it with:\n";
    readme << "   ```bash\n";
