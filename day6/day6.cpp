...
std::vector<std::vector<unsigned long long>> transpose(const std::vector<std::vector<unsigned long long>>& input) {
    if (input.empty()) {
        return {};
    }
    ...
    std::vector<std::vector<unsigned long long>> transposed(cols, std::vector<unsigned long long>(input.size()));
