...
std::vector<int> calculateWidths(std::ifstream &file, int columns,
...
transpose(const std::vector<std::vector<std::string>> matrix) {
  if (matrix.empty())
    return {};
...
convert(const std::vector<std::vector<std::string>> &matrix,
