...
long int rectSize(Point &a, Point &b) {
  ...
  return std::sqrt(pow((bottomRight.x - topLeft.x + 1), 2)) *
         std::sqrt(pow((bottomRight.y - topLeft.y + 1), 2));
}
...
bool isValid(const std::pair<Point, Point> &rect,
             const std::vector<std::pair<Point, Point>> &allLines) {

  for (size_t i = 0; i < allLines.size(); i++) {
    if (isPointInRect(allLines[i].first, rect) ||
        isPointInRect(allLines[i].second, rect)) {
      return false;
    }
    if (allLines[i].first.x == allLines[i].second.x) {
      Point top = {std::min(allLines[i].first.x, allLines[i].second.x),
                   std::min(allLines[i].first.y, allLines[i].second.y)};
      Point bot = {std::max(allLines[i].first.x, allLines[i].second.x),
                   std::max(allLines[i].first.y, allLines[i].second.y)};
      ...
      Point left = {std::min(allLines[i].first.x, allLines[i].second.x),
                    std::min(allLines[i].first.y, allLines[i].second.y)};
      Point right = {std::max(allLines[i].first.x, allLines[i].second.x),
                     std::max(allLines[i].first.y, allLines[i].second.y)};
      long int yMin = std::min(rect.first.y, rect.second.y);
      long int yMax = std::max(rect.first.y, rect.second.y);
      long int xMin = std::min(rect.first.x, rect.second.x);
      long int xMax = std::max(rect.first.x, rect.second.x);
      if (left.x <= xMin && right.x >= xMax && left.y > yMin &&
          right.y < yMax) {
        return false;
      }
    }
...
void separateLines(const std::vector<Point> &points,
